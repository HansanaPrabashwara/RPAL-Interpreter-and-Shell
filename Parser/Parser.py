...
class Parser:
    """
    Parse the tokens list provided by the screener
    """
    ...
    def printAST(self):
        """
        Print the Abstract Syntaxt Tree for the given source code
        """
        tree = self.stack[0]
        def traverse(root, n):
            print("."*n + root.data)
    ...
    def build_tree(self,transduction, n):
        """Build the Abstract Syntaxt Tree

        Args:
            token (String): Transduction grammer
            n (int): Number of children under the transduction node
        """
        ...
        if len(self.stack) >= n:
            ...
            # print(f"PARSER : Stack size {len(self.stack)} is less than {n} , cannot create node {self.token}")


    def read(self, token):
        """Reads the provided token and increment the current index

        Args:
            token (Token): Token to be read
        """
    ...
    def parse(self):
        """
        Initiate the parsing proceedure
        """
    ...
    def E(self):
        ...
        if self.next_token.val == "let":
            ...
            if self.next_token.val == "in":
                ...
                # print(f"Error : got '{self.next_token.val}' expected 'in' ")
    ...
    def Rn(self):
        ...
        if self.next_token.val == "true":
            ...
            if self.next_token.val != ")":
                ...
                # print("Error: Expected a matching )")
    ...
    def Db(self):
        ...
        if self.next_token.key == "IDENTIFIER":
            if len(self.token_list) > self.index+1:
                #cannot decide the path with just next_token becuase of V1 also starts with <IDENTIFIER>. need to check the token after next_token : LL(2)
                if self.token_list[self.index+1].val == "=" or self.token_list[self.index+1].val == ",": ###############################################
                    ...
                    # print("Error: '=' or an identifier expected")
    ...
    def printPreOrder(self, node):
        """Helper method for the get_pre_ordered_list
